...
function resizeApp(){
	if(!begin_entered){
		cont_1 = document.getElementById('begin_page');
		...
		$('body').css('height',(bodyheight)+'px');
		$('body').css('background-size','100% '+(bodyheight)+'px');
		var topRefer = ($("#responsive_container").height()*scale)+$("#direction_text").height()+$("#popUp_container").height();
		$('#popUp_container').css('top',topRefer+'px');
		$('#whole_container').css('height',($("#responsive_container").height()*scale)+$("#direction_text").height()+$("#popUp_container").height());
